...
class InvitesController extends AppController {
	...
	var $uses = array('Event', 'Invite');
        ...
        function beforeFilter(){
                $this->Auth->allow('index','view','sendInvite', 'optOut');
		parent::beforeFilter();
        }
	...
	function admin_edit($id = null, $eventUUID = null) {
		...
		if (empty($this->request->data)) {
			$this->request->data = $this->Invite->find('first',array('conditions'=>array('Invite.id'=>$id)));
		}
		$event = $this->Invite->Event->isOwner($this->request->data['Invite']['event_id'],'id');
	...
	function admin_resend($id=null,$eventUUID = null){
		...
		if( $result = $this->Invite->save($invite)){
			...
			$this->Session->setFlash(__('Scheduled Invite To Be Resent.'));
	...
	function admin_delete($id = null,$eventUUID = null) {
		if (!$id) {
			$this->Session->setFlash(__('Invalid invite given, is someone being nosy....'));
			if($eventUUID) $this->redirect(array('controller'=>'invites', 'action'=>'view_event',$eventUUID));
			else return false;
		}
	...
	function admin_bulkUpdate(){
		$this->log("Data I got:".print_r($this->request->data,1),'debug');
		if(!empty($this->request->data)){
			if(isset($this->request->data['Invite']['cmd'])){
				$this->log("Data is not Empty",'debug');
				switch($this->request->data['Invite']['cmd']){
					case 'resend':
						$this->log("Doing Bulk Resend",'debug');
						if(isset($this->request->data['Invite']['id']) && is_array($this->request->data['Invite']['id']) ){
							$result = '';
							foreach($this->request->data['Invite']['id'] as $invite){
					...
					default:
						$this->log("Doing Bulk Delete",'debug');
						$this->Session->setFlash(__('Invalid Bulk Option'));
						break;
				}
			}
		}
		$this->redirect(array('controller'=>'invites', 'action'=>'view_event',$this->request->data['event']['id']));
